...
export class PCFRegistrationPage {
  ...
  readonly confirmPasswordInput = $('input[name="confirmPassword"], input[name="password_confirmation"], [data-testid="confirm-password"]');
  ...
  readonly registrationForm = $('form[data-testid="registration-form"], .registration-form, form:has(input[name="firstName"])')
    .with({
  ...
  async registerWithTestUser(user: {
    firstName: string;
    lastName: string;
    email: string;
    password: string;
